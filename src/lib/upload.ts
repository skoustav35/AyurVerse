...
async function legacyLane(file: File, onProgress?: ProgressFn): Promise<string> {
  onProgress?.(15);
...
export async function uploadMedia(rawFile: File, onProgress?: ProgressFn): Promise<string> {
  ...
  try {
    ...
    if (/stone|heavier|large/i.test(msg)) throw new Error(msg);
    try {
      return await legacyLane(file, onProgress);
    } catch (err2) {
