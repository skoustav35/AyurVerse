...
import { Bookmark, CalendarDays, Camera, LogOut, PencilLine, Sprout, Users, Wand2, Code2, Wallet } from 'lucide-react';
...
import DeveloperView from '../studio/DeveloperView';
...
type StudioSub = 'analytics' | 'payouts' | 'developer';
  ...
  return (
    <div className="w-full max-w-[820px] mx-auto pb-14">
      ...
      <div className="px-4 lg:px-6 -mt-10 relative z-10">
        ...
        {/* Top-level sub-tabs */}
        <div className="flex rounded-2xl border border-sand-300 bg-parchment p-1.5 mt-6 shadow-[inset_0_1px_0_rgb(255_252_240/0.5)]">
          {(
            [
              { id: 'weave', label: 'Your Weave', icon: Sprout, hint: 'what you have grown' },
              { id: 'apothecary', label: 'Apothecary', icon: Bookmark, hint: 'what you have kept' },
              { id: 'studio', label: 'Studio', icon: Wand2, hint: 'analytics · payouts · developer' },
            ] as const
          ).map((t) => (
            <button
              key={t.id}
              onClick={() => setSubTab(t.id)}
              className={`relative flex-1 rounded-xl px-4 py-2.5 transition-colors ${subTab === t.id ? '' : 'hover:bg-sand-200/50'}`}
            >
              {subTab === t.id && (
                <motion.span
                  layoutId="you-subtab"
                  className="absolute inset-0 rounded-xl bg-gradient-to-r from-neem-800 to-neem-700 shadow-[0_8px_20px_-8px_rgba(18,41,28,0.6)]"
                  transition={{ type: 'spring', stiffness: 360, damping: 30 }}
                />
              )}
              <span className="relative z-10 flex items-center justify-center gap-2">
                <t.icon size={15} className={subTab === t.id ? 'text-gold-400' : 'text-ink-400'} />
                <span className={`text-[13px] font-semibold ${subTab === t.id ? 'text-parchment' : 'text-ink-600'}`}>
                  {t.label}
                </span>
                <span className={`text-[9.5px] uppercase tracking-wider ${subTab === t.id ? 'text-gold-300/80' : 'text-ink-400'}`}>
                  {t.hint}
                </span>
              </span>
            </button>
...
function StudioView({ active, onChange }: { active: StudioSub; onChange: (s: StudioSub) => void }) {
  ...
  return (
    <div className="mt-5">
      <div className="inline-flex items-center gap-1 rounded-full border border-sand-300 bg-parchment p-1 text-[11.5px] font-semibold">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => onChange(t.id)}
            className={`relative px-4 py-1.5 rounded-full transition-colors flex items-center gap-1.5 ${
              active === t.id ? 'text-parchment' : 'text-ink-600 hover:text-neem-800'
            }`}
          >
            {active === t.id && (
              <motion.span layoutId="studio-pill" className="absolute inset-0 rounded-full bg-neem-800" transition={{ type: 'spring', stiffness: 360, damping: 30 }} />
            )}
      ...
      {active === 'developer' && <DeveloperView />}
    </div>
  );
}
