...
import { useComments, useFollows, useToggleFollow, useToggleLike, useToggleSave } from '../../hooks/queries';
...
/* The circle, over the reel — a bottom sheet holding the full nested thread. */
function ReelCommentSheet({
  post,
  open,
  onClose,
}: {
  ...
  // the thread is only fetched once the sheet is summoned — a deck of 80 reels
  // must never fan out 80 comment requests
  const { data: comments, isLoading } = useComments(open ? post.id : null);
      ...
      {open && (
        <>
          <motion.div
            ...
            className="absolute inset-0 z-[14] bg-neem-950/55 backdrop-blur-[2px]"
          />
          <motion.section
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            ...
            className="absolute inset-x-0 bottom-0 z-[15] max-h-[72%] flex flex-col rounded-t-3xl bg-parchment shadow-[0_-18px_46px_-16px_rgba(0,0,0,0.65)]"
            aria-label="Reel discourse"
          >
            <header className="shrink-0 px-4 pt-3 pb-2.5 border-b border-sand-300/70">
              <span className="mx-auto mb-2.5 block w-10 h-1 rounded-full bg-sand-400" />
              <div className="flex items-center justify-between gap-3">
                <h3 className="font-display font-semibold text-[15px] text-ink-900">
                  Discourse &amp; Reflections
                  <span className="ml-1.5 text-[12px] font-normal text-ink-500 tabular-nums">
                    {compact(post.comments_count)}
                  </span>
                </h3>
                <button
                  onClick={onClose}
                  className="p-1.5 rounded-full text-ink-500 hover:bg-sand-200/80 transition-colors"
  ...
  return (
    <div className="h-full w-full snap-start relative flex items-center justify-center bg-neem-950">
      <div className="relative h-full w-full lg:h-[96%] lg:w-auto lg:aspect-[9/16] lg:rounded-[28px] overflow-hidden lg:border lg:border-gold-500/25 lg:shadow-[0_30px_90px_-30px_rgba(12,27,19,0.8)]">
        ...
        <div className="absolute bottom-0 inset-x-0 pl-4 pr-20 pb-6 z-[6]">
          ...
          {post.title && (
            <button
              ...
              className="pointer-events-auto mt-2.5 block text-left text-[13.5px] font-semibold text-parchment leading-snug drop-shadow hover:text-gold-300 transition-colors"
            >
              {post.title}
            </button>
          )}
        ...
        {/* the circle, summoned over the reel */}
        <ReelCommentSheet post={post} open={commentsOpen} onClose={() => setCommentsOpen(false)} />
      </div>
    </div>
  );
}
...
export default function ReelsView() {
  ...
  return (
    <div className="relative h-[calc(var(--vvh,100dvh)-118px)] lg:h-[calc(100vh-24px)] bg-[#07110a] lg:rounded-[28px] lg:mx-2 overflow-hidden lg:border lg:border-gold-500/20">
      ...
      {isLoading && (
        <div className="h-full grid place-items-center bg-[#07110a]">
          <div className="text-center">
            <Clapperboard className="mx-auto text-gold-500 animate-pulse" size={30} />
            <p className="font-display italic text-[#f8f3e5]/80 mt-3">Rolling the projector…</p>
          </div>
        </div>
      )}

      {isError && (
        <div className="h-full grid place-items-center text-center px-8 bg-[#07110a]">
          <div>
            <p className="font-display text-lg text-[#f8f3e5]">The projector lamp flickered out</p>
      ...
      {reels && reels.length === 0 && (
        <div className="h-full grid place-items-center px-8 text-center bg-[#07110a]">
          <div>
            <Clapperboard className="mx-auto text-gold-400" size={32} />
            <p className="font-display italic text-[#f8f3e5] text-lg mt-4">No vertical reels in the tin yet.</p>
            <p className="text-[13px] text-sand-400/80 mt-2 max-w-xs mx-auto">Record a 9:16 clip from the Reels tab on your phone — it will weave in here.</p>
          </div>
        </div>
      )}
