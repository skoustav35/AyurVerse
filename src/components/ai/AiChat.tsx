...
export default function AiChat({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="h-full flex flex-col bg-parchment-deep/40">
      ...
      <header className="shrink-0 px-4 py-3 border-b border-sand-300/70 bg-gradient-to-b from-parchment/80 to-transparent">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="grid place-items-center w-9 h-9 rounded-full bg-gradient-to-br from-[#1b3d29] to-[#11281a] ring-1 ring-gold-500/50 shrink-0">
              <Sparkles size={16} className="text-gold-300" />
            </span>
            <div className="min-w-0">
              <p className="font-display font-semibold text-[15.5px] text-ink-900 leading-tight">Vaidya</p>
      ...
      {/* Messages */}
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto no-scrollbar px-4 py-4 space-y-4">
        {messages.length === 0 && !pending && (
          <div className="relative h-full flex flex-col items-center justify-center text-center px-2">
            ...
            <span className="relative grid place-items-center w-14 h-14 rounded-2xl bg-gradient-to-br from-[#1b3d29] to-[#11281a] ring-1 ring-gold-500/50 shadow-md">
              <Sparkles size={22} className="text-gold-300" />
            </span>
            <p className="relative font-display font-semibold text-[19px] text-ink-900 mt-4">Namaste, I am Vaidya</p>
        ...
        {pending && (
          <div className="flex items-center gap-2 pl-1">
            <span className="grid place-items-center w-8 h-8 rounded-full bg-gradient-to-br from-[#1b3d29] to-[#11281a] ring-1 ring-gold-500/50 shrink-0">
...
function MessageBubble({ msg, liveCount }: { msg: ChatMsg; liveCount?: number }) {
  ...
  if (msg.role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-tr-md bg-gradient-to-br from-[#1b3d29] to-[#11281a] text-[#f8f3e5] px-4 py-2.5 shadow-[0_6px_16px_-8px_rgba(0,0,0,0.6)] ring-1 ring-gold-500/20">
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="grid place-items-center w-8 h-8 rounded-full bg-gradient-to-br from-[#1b3d29] to-[#11281a] ring-1 ring-gold-500/50 shrink-0 mt-0.5">
        <Sparkles size={14} className="text-gold-300" />
      </span>
