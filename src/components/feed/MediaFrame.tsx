...
interface MediaFrameProps {
  ...
  onDoubleTap?: () => void;
  onSingleTap?: () => void;
  /** Feed tiles crop to a uniform 3:2 band; a reader should show the whole frame. */
...
export default function MediaFrame({
  ...
  // The frame the medium actually has — clamped so a very tall portrait can't
  // push the reader column into an endless strip.
  const shownRatio = naturalAspect && ratio ? Math.min(Math.max(ratio, 0.62), 2.2) : null;
  ...
  return (
    <motion.div
      ...
      style={{
        ...(tiltable ? { transform } : {}),
        aspectRatio: shownRatio ? String(shownRatio) : '3 / 2',
      }}
      transition={{ type: 'spring', stiffness: 200, damping: 22 }}
      className={`relative w-full select-none overflow-hidden cursor-pointer will-change-transform bg-sand-200 dark:bg-[linear-gradient(140deg,#1d4230,#0e2118)] ${
