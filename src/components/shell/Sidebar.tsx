...
import { Clapperboard, Flame, Home, LibraryBig, LogOut, Plus, Send, CircleUserRound, Bell, UsersRound, Moon, Sun } from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[248px] xl:w-[272px] shrink-0 h-full flex flex-col border-r border-gold-500/20 bg-black/40 backdrop-blur-2xl relative z-10 shadow-[8px_0_35px_-10px_rgba(0,0,0,0.8)]">
      ...
      <div className="shrink-0 border-t border-gold-500/20 px-4 pt-3 pb-5 space-y-2.5">
        <button
          onClick={toggleTheme}
          className="w-full flex items-center justify-between px-3.5 py-2 rounded-xl border border-gold-500/20 bg-gold-500/10 hover:bg-gold-500/20 text-[12px] font-medium text-ink-700 transition-colors"
        ...
        >
          <span className="flex items-center gap-2">
            {theme === 'dark' ? (
              ...
              <Sun size={14} className="text-saffron-600" />
            )}
            <span className="text-ink-800 font-semibold">
              {theme === 'dark' ? 'Nocturnal Atelier' : 'Parchment Light'}
            </span>
          </span>
          <span className="text-[9.5px] uppercase font-bold tracking-wider text-gold-600 bg-gold-400/20 px-2 py-0.5 rounded-full">
