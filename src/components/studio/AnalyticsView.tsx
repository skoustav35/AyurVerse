    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex items-center gap-3 text-[11.5px] text-ink-600">
          ...
          <span className="inline-flex items-center gap-1"><Eye size={12} />{compact(post.views_count)}</span>
  ...
  return (
    <div className="px-4 lg:px-6 pt-4 pb-14 space-y-6">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <KpiCard icon={Eye} label="Views" value={t?.views} loading={isLoading} tone="neem" />
        <KpiCard icon={MessageCircle} label="Reflections" value={t?.comments} loading={isLoading} />
        <KpiCard icon={Users} label="Channels (followers)" value={t?.followers} loading={isLoading} tone="gold" />
      </section>
...
function KpiCard({ icon: Icon, label, value, loading, tone = 'ink' }: { icon: any; label: string; value: number | string | undefined; loading?: boolean; tone?: 'ink' | 'terra' | 'neem' | 'gold' }) {
  const tones = {
    ...
    neem: 'text-neem-700',
    gold: 'text-saffron-700',
  } as const;
  return (
    <div className="card-warm p-3.5">
      <Icon size={16} className={tones[tone]} />
      <p className="text-[10.5px] font-semibold uppercase tracking-[0.16em] text-ink-500 mt-2">{label}</p>
      <p className={`font-display font-semibold text-[22px] mt-0.5 ${loading ? 'text-ink-300' : 'text-ink-900'}`}>
