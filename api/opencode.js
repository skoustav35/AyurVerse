...
// OpenCode Zen gateway (https://opencode.ai/zen/v1). The legacy AVS gateway
// host is dead; Zen is the live, OpenAI-compatible door. Env override kept so
// a rotated key/base never needs a code change.
const BASE = process.env.GATEWAY_BASE_URL || 'https://opencode.ai/zen/v1';
const FALLBACK_KEY = 'sk-Fc4ac08nEJWm8yY51omw6K8uOGJdgcjhLq9ez2KSfsc4akhTu8jvsPSw1yeHNilk';
// Zen serves models over the OpenAI Responses API (/responses). The cascade
// still tries /chat/completions and /messages for compatibility.
const HOUSE_EFFORT = 'high';
// big-pickle is the house mind (free tier). Free ids occasionally 500 when the
// upstream pool is saturated, so we keep a couple of fallbacks; a paid model
// is used automatically once the workspace has billing enabled. Override the
// order with GATEWAY_MODEL in env.
const MODEL_CANDIDATES = [
  process.env.GATEWAY_MODEL,
  process.env.SOCIETY_MODEL,
  'big-pickle',
  'mimo-v2.5-free',
  'gpt-5.4-nano',
  'gpt-5-nano',
...
let lastBlockedAt = 0;
const BLOCK_CACHE_TTL = 300000; // 5 minutes
...
class AuthBlocked extends Error {
  constructor(status, detail) {
    super(`gateway rejected credentials (${status})`);
    this.code = 'AUTH_BLOCKED';
    this.status = status;
    this.detail = detail;
  }
}
...
async function attempt({ key, path, authHeader, model, form, system, messages, maxTokens, temperature }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 402 || res.status === 403) throw new AuthBlocked(res.status, txt.slice(0, 160));
...
export async function chat({ system, messages, maxTokens = 2000, temperature = 0.7 }) {
  ...
  if (lastBlockedKey === key && Date.now() - lastBlockedAt < BLOCK_CACHE_TTL) {
    const err = new Error('gateway credentials rejected (cached)');
    err.code = 'GATEWAY_DOWN';
    err.gateway_errors = ['upstream key is blocked or exhausted'];
    throw err;
  }
  ...
  plans.push({ path: '/responses', authHeader: 'Authorization', model: MODEL_CANDIDATES[0], form: 'responses' });
  plans.push({ path: '/messages', authHeader: 'x-api-key', model: MODEL_CANDIDATES[0], form: 'anthropic' });

  const errors = [];
  let sawEmpty = false;        // gateway reachable but reasoning ate the budget
  let sawRateLimit = false;    // key is over its usage / rate limit
  let sawAuthBlocked = false;  // key rejected with 401/402/403

  for (const p of plans) {
    if (sawAuthBlocked || sawRateLimit) break;
    ...
    for (let i = 0; i < budgets.length; i++) {
      try {
        ...
        if (err.code === 'AUTH_BLOCKED') {
          sawAuthBlocked = true;
          lastBlockedKey = key;
          lastBlockedAt = Date.now();
          errors.push(`${p.form}@${p.authHeader}:${p.model} → ${err.status} ${err.detail || ''}`);
          break;
        }
        if (err.code === 'EMPTY') {
          sawEmpty = true;
...
export { BASE, MODEL_CANDIDATES, MODEL_CANDIDATES as MODELS };
