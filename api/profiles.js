...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const lim = Math.min(parseInt(limit, 10) || 8, 24);
      const { data, error } = await supabase.from('profiles').select('*').order('id', { ascending: true }).limit(lim);
      if (error) throw error;
      return res.status(200).json(data);
    }
    ...
    if (req.method === 'POST') {
      ...
      if (existing) {
        ...
        const { data, error } = await supabase.from('profiles').update(clean).eq('user_id', user.id).select().single();
        if (error) throw error;
        return res.status(200).json(data);
      }
