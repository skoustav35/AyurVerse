...
async function attachFlags(items, user) {
  ...
  items.forEach((p) => {
    p.media_urls = Array.isArray(p.media_urls) && p.media_urls.length
      ? p.media_urls
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const rawUrls = Array.isArray(body.media_urls) && body.media_urls.length
        ? body.media_urls.map(String)
        : body.media_url ? [String(body.media_url)] : [];
      const primaryUrl = rawUrls[0] || null;

      if (kind === 'visual' && !primaryUrl) return res.status(400).json({ error: 'A visual post needs media' });
      ...
      const row = {
        ...
        media_urls: rawUrls,
        media_type: body.media_type === 'video' ? 'video' : primaryUrl ? 'image' : null,
    ...
    if (req.method === 'DELETE') {
      ...
      const { data: existing } = await db
        .from('posts')
        .select('author_id, media_url, media_urls')
        .eq('id', id)
        .maybeSingle();
      if (!existing) return res.status(404).json({ error: 'Post not found' });
      ...
      // every carousel slide is burned, not just the cover frame
      const owned = [
      ...
      const paths = [
        ...new Set(
          owned
            .map((u) => String(u).match(/\/object\/public\/media\/(.+)$/))
            .filter(Boolean)
            .map((m) => decodeURIComponent(m[1])),
        ),
      ];
      if (paths.length) mediaBucket.remove(paths).then(() => {}).catch(() => {});
