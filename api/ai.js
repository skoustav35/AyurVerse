...
import { polishScribeText } from './ayurvedic-sage.js';
...
const PROMPTS = {
  ...
  vaidya_seal:
    "You are the Chief Clinical Vaidya and Scholarly Physician of AyurVerse. Analyze the manuscript text and output an authentic, highly regarded clinical seal in structured Markdown with: (1) ### Doshic Suitability (Vata, Pitta, Kapha pacification/aggravation), (2) ### Clinical Physiological Commentary (mechanism of Agni, Dhatus, Srotas), (3) ### Classical Anupana (carrier vehicle), (4) ### Classical Shloka / Canonical Citation (from Charaka Samhita or Ashtanga Hridaya), (5) ### Contraindications. Use dignified Sanskrit-scholarly medical terminology with English explanations.",
};
...
const LIMITS = { caption: 1600, summary: 1200, manuscript: 8000, vaidya_seal: 3500 };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const mode = ['caption', 'summary', 'manuscript', 'vaidya_seal'].includes(req.body?.mode) ? req.body.mode : null;
    if (!mode) return res.status(400).json({ error: 'mode must be caption, summary, manuscript or vaidya_seal' });
    ...
    if (!hasKey()) {
      return res.status(200).json({
        text: polishScribeText({ mode, text, title }),
    ...
    const userPrompt =
      mode === 'manuscript'
        ...
        : mode === 'caption'
          ? `Caption draft:\n${text}`
          : mode === 'vaidya_seal'
            ? `Manuscript Title: ${title || '(untitled)'}\n\nContent to analyze and certify:\n${text}`
    ...
    try {
      ...
      if (err.code === 'GATEWAY_DOWN' || err.code === 'EMPTY' || err.code === 'RATE_LIMIT' || err.code === 'NO_KEY') {
