...
import { runVaidyaAgent } from './ayurvedic-sage.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!hasKey()) {
      const outcome = await runVaidyaAgent({
        messages: working,
        userId,
        user,
        meta,
    ...
    for (let step = 0; step < MAX_STEPS; step++) {
      ...
      try {
        ...
        if (err.code === 'RATE_LIMIT' || err.code === 'GATEWAY_DOWN' || err.code === 'EMPTY') {
          console.warn(`[agent] Upstream ${err.code} — delegating to Ayurvedic Sage engine`);
          const outcome = await runVaidyaAgent({
            messages: working,
            userId,
            user,
