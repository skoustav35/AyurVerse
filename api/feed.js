...
export default async function handler(req, res) {
  ...
  try {
    ...
    let dinacharyaMode = true;
    if (user) {
      const { data: prefs } = await db.from('user_prefs').select('*').eq('user_id', user.id).maybeSingle();
      ...
      dinacharyaMode = prefs?.dinacharya_mode !== false;
    }
    ...
    const dinacharyaTags = dinacharyaMode
      ? localHour >= 4.5 && localHour < 9
        ? new Set(['dinacharya', 'pranayama', 'meditation', 'chants', 'herbal-tea', 'morning'])
        : localHour >= 11 && localHour < 14.5
          ? new Set(['forge', 'deep-lore', 'agni', 'rasashastra', 'recipes'])
          : localHour >= 17.5 && localHour < 20.5
            ? new Set(['sandhya', 'vata', 'restorative', 'herbal', 'reflection'])
            : (localHour >= 21.5 || localHour < 4.5)
              ? new Set(['nidra', 'sleep', 'rest', 'ojas', 'wind-down'])
              : new Set()
      : new Set();
    const scored = posts.map((p) => {
      ...
      const fitsDinacharya = tagList.some((t) => dinacharyaTags.has(t));
      if (hasBoosted) aff += 6;
      if (fitsDinacharya) aff += 1.8;
      ...
      else if (fitsDinacharya) reason = 'Aligned with your Dinacharya rhythm';
    ...
    return res.status(200).json({
      ...
      meta: { personalized: signals.length > 0, taste, dinacharya_mode: dinacharyaMode },
    });
  } catch (err) {
