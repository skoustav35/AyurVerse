...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Lane A: signed-URL upload (big videos / high-res photos — files never pass through the function)
    if (req.body?.direct) {
      ...
      // rehost the signed URL to the streaming storage gateway (no legacy buffering)
    ...
    // Lane B: legacy base64 (small files only — request size is capped by the platform)
