...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data: posts } = await db.from('posts').select('*').in('id', ids).order('id', { ascending: false });
      const items = posts || [];
    ...
    if (req.method === 'POST') {
      ...
      const { data: mem } = await db.from('group_members').select('role').eq('group_id', gid).eq('user_id', user.id).maybeSingle();
